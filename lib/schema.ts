...
// License Verification Logs - Track all license verification attempts
export const licenseVerificationLogs = mysqlTable("license_verification_logs", {
  id: varchar("id", { length: 255 }).primaryKey(),
  clientId: varchar("client_id", { length: 255 }).notNull(),
  licenseKey: varchar("license_key", { length: 255 }).notNull(),
  requestDomain: varchar("request_domain", { length: 255 }).notNull(), // Domain making the request
  requestIp: varchar("request_ip", { length: 45 }), // IPv4 or IPv6
  userAgent: text("user_agent"), // Browser/client info
  verificationStatus: varchar("verification_status", { length: 20 }).notNull(), // valid, invalid, expired, suspended
  errorMessage: text("error_message"), // Error details if verification failed
  responseTime: int("response_time").default(0), // Response time in milliseconds
  createdAt: datetime("created_at").default(sql`CURRENT_TIMESTAMP`),
});

// Client Settings - Store client-specific configurations
export const clientSettings = mysqlTable("client_settings", {
  id: varchar("id", { length: 255 }).primaryKey(),
  clientId: varchar("client_id", { length: 255 }).notNull(),
  settingKey: varchar("setting_key", { length: 100 }).notNull(), // e.g., 'theme_color', 'max_products', 'features_enabled'
  settingValue: text("setting_value").notNull(), // JSON string for complex values
  settingType: varchar("setting_type", { length: 50 }).default("string"), // string, boolean, number, json, array
...
// License Usage Analytics - Track usage patterns and statistics
export const licenseUsageAnalytics = mysqlTable("license_usage_analytics", {
  id: varchar("id", { length: 255 }).primaryKey(),
  clientId: varchar("client_id", { length: 255 }).notNull(),
  date: varchar("date", { length: 10 }).notNull(), // YYYY-MM-DD format
  totalRequests: int("total_requests").default(0), // Total API requests made
  uniqueVisitors: int("unique_visitors").default(0), // Unique IPs accessing the site
  pageViews: int("page_views").default(0), // Total page views
  orders: int("orders").default(0), // Total orders placed
  revenue: decimal("revenue", { precision: 10, scale: 2 }).default('0.00'), // Revenue generated
  activeUsers: int("active_users").default(0), // Active users on the platform
  storageUsed: decimal("storage_used", { precision: 12, scale: 2 }).default('0.00'), // Storage used in MB
  bandwidthUsed: decimal("bandwidth_used", { precision: 12, scale: 2 }).default('0.00'), // Bandwidth used in MB
  createdAt: datetime("created_at").default(sql`CURRENT_TIMESTAMP`),
});

// SaaS Relations
export const saasClientsRelations = relations(saasClients, ({ many }) => ({
  verificationLogs: many(licenseVerificationLogs),
  settings: many(clientSettings),
  analytics: many(licenseUsageAnalytics),
}));

export const licenseVerificationLogsRelations = relations(licenseVerificationLogs, ({ one }) => ({
  client: one(saasClients, {
    fields: [licenseVerificationLogs.clientId],
    references: [saasClients.id],
  }),
}));
...
export const licenseUsageAnalyticsRelations = relations(licenseUsageAnalytics, ({ one }) => ({
  client: one(saasClients, {
    fields: [licenseUsageAnalytics.clientId],
    references: [saasClients.id],
  }),
}));